...
def main():
    ...
    elif election == "Pasar de Imagen a Texto":
        st.subheader("Pasar de Imagen a Texto")
        imagen = st.file_uploader("Carga una imagen", type=["png", "jpg", "jpeg"])
        if imagen is not None:
            st.image(cargar_imagen(imagen), width=250, caption="Imagen cargada para extraer texto")
            texto_extraido = extraer_texto_de_imagen(Image.open(imagen))
            st.text_area("Texto Extraído de la Imagen", texto_extraido, height=300)

            if st.button("Generar documento de texto"):
                buffer, nombre_archivo = crear_documento_word(texto_extraido)
                st.download_button(
                    label="Descargar Archivo Word",
                    data=buffer,
